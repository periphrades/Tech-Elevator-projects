...
public class CampgroundCLI {
	...
	private void parkInfoMenu(Park park) {
		...
		while (true) {
			...
			if (menuChoice.equals("View Campgrounds")) {
				...
				String subMenuChoice = (String) parkMenu.getChoiceFromOptions(subMenuOptions);
	...
	private boolean makeReservation(Park park) {
		while (true) {
			...
			while (campChoice == null) {
				...
				campChoice = parkMenu.getUserChoiceFromAllowables(campgrounds, allowableChoices);
			}
			...
			boolean notLegitDates = true;
			LocalDate arrivalDate = null;
			LocalDate departureDate = null;
			
			while (notLegitDates) {
			
				arrivalDate = parkMenu.getDateInputFromUser("What is the arrival date? (MM/DD/YYYY) ");
				...
				departureDate = parkMenu.getDateInputFromUser("What is the departure date? (MM/DD/YYYY) ");
			...
			boolean reservationMade = pickSite(chosenCampground, arrivalDate, departureDate);
	...
	private boolean pickSite(Campground camp, LocalDate arrive, LocalDate depart) {
		parkMenu.printOneLine("Results Matching Your Search Criteria\n");
		String str = String.format("%-12s%-12s%-13s%-14s%-10s%-8s", "Site No.", "Max Occup.", 
				"Accessible?", "Max RV Length", "Utility", "Cost");
		parkMenu.printOneLine(str);
		List <Object> availableSites = siteDAO.getAvailableSites(camp, arrive, depart);
		
		if (availableSites.size() == 0) {
			parkMenu.printOneLine("There are no available sites.\n");
			parkMenu.printOneLine("Please pick a new campground or enter alternate dates.");
			return false;
		}
		...
		while (siteChoice == null) {
			parkMenu.printOneLine("");
			parkMenu.printOneLine("Which site should be reserved (enter 0 to cancel)? ");
			siteChoice = parkMenu.getUserChoiceFromAllowables(availableSites, allowableChoices);
		}
		...
		String reservationName = parkMenu.getStringFromUser("What name should "
				+ "the reservation be made under? ");
		
		long confirmation = reservationDAO.createReservation(chosenSite, reservationName, arrive, depart);
		parkMenu.printOneLine("The reservation has been made and the confirmation id is: " + confirmation);
		parkMenu.printOneLine("\n");
		return true;
	}
	
	private List<Object> showCampGrounds(Park park) {
		String str = String.format("%-5s%-35s%-17s%-17s%-11s", " ", "Name", "Open", "Close", "Daily Fee");
		parkMenu.printOneLine(str);
