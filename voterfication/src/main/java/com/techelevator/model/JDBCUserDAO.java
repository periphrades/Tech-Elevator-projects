...
public class JDBCUserDAO implements UserDAO {
	...
	public Object getUserByUserName(String userName) {
		String sqlSearchForUsername = "SELECT * " + "FROM app_user " + " where user_name = ? ";
	...
	public Object getUserById(int userId) {
		String sqlSearchForId = "SELECT * " + "FROM app_user where " + "id = ? ";

		SqlRowSet user = jdbcTemplate.queryForRowSet(sqlSearchForId, userId);
		User thisUser = null;
		if (user.next()) {
			thisUser = new User();
			thisUser.setUserName(user.getString("user_name"));
